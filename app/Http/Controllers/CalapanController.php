...
class CalapanController extends Controller
{
    public function profiling()
    {
        ...
        return view('user.calapan.profiling', compact('data', 'data2'));
    }
    public function profileSave(Request $request)
    {
        $data = new CalapanProfiles();
    ...
    public function viewCalapanEvent($id)
    {
        $currentDate = Carbon::now('Asia/Manila')->toDateString();
        $data = CalapanEvent::where('date', '>=', $currentDate)->get();
        $data2 = CalapanEvent::where('date', '<', $currentDate)->get();
        $event = CalapanEvent::where('id', $id)->get();
        return view('user.calapan.view-calapan-event', compact('event', 'data', 'data2'));
    }
}
